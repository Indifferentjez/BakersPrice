              ...
              <tbody>
                {rows.map((q) => (
                  <tr key={q.id}>
                    ...
                    <td data-label="Mode">{q.mode}{q.is_estimate ? <span className="badge REQUIRES_TESTING" style={{ marginLeft: 6 }}>estimate</span> : ''}</td>
                    ...
                    <td data-label="">
                      <div className="row-actions" style={{ justifyContent: 'flex-end' }}>
                        ...
                        <button className="btn danger sm" onClick={async () => { await api.del(`/api/quotes/${q.id}`); load(); }}>Delete</button>
                      </div>
                    </td>
                  </tr>
                ))}
